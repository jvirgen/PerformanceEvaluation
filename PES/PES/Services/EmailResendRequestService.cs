...
{
    public class EmailResendRequestService
    {
        private PESDBContext dbContext = new PESDBContext();
    
       
        private SmtpClient client;
        public EmailResendRequestService()
        {
        ...
        public List<ResendRequestViewModel> GetEmail(int HeaderRequestId)
        {
            List<ResendRequestViewModel> dataRequests;
            ResendRequestViewModel data;
            ...
            try
            {
                ...
                using (OracleConnection db = dbContext.GetDBConnection())
                {
                    ...
                    {
                        OracleDataReader reader = command.ExecuteReader();
                        dataRequests = new List<ResendRequestViewModel>();
                        while (reader.Read())
                        {
                            data = new ResendRequestViewModel();
                            data.EmployeeEmail = Convert.ToString(reader["EMAIL"]);
                            data.ManagerEmail = Convert.ToString(reader["managerEmail"]);
